...
int main(int argc, char **argv) {
  ...
  // join left and right
  cv::Mat steropair(left.rows, left.cols * 2, CV_8UC1);
  cv::Mat left_roi = steropair(cv::Rect(0, 0, left.cols, left.rows));
  left.copyTo(left_roi);
  cv::Mat right_roi = steropair(cv::Rect(left.cols, 0, left.cols, left.rows));
  right.copyTo(right_roi);
  cv::imshow("stereo pair", steropair);
